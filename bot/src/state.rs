...
pub struct AppState {
    ...
    /// Unix time (ms) of the most recent WS heartbeat we sent to clients.
    /// Surfaced on REST /api/status.lastHeartbeatAt.
    ...
    /// Unified outbound WS channel. bot_loop produces DTO-shaped frames;
    /// api::ws forwards them to subscribers as JSON.
    pub ws_tx: broadcast::Sender<WsFrame>,
...
impl AppState {
    pub fn new(
        ...
        Arc::new(Self {
            ...
            last_heartbeat_at: AtomicI64::new(0),
    ...
    /// Record that a client-facing heartbeat just went out. Returns the
    /// timestamp so the caller can also build a WsFrame::Heartbeat.
    pub fn mark_heartbeat(&self) -> i64 {
        let now = crate::api::dto::now_unix_ms();
